...
const oddValues = (arr) => {
  const result = arr.filter(oddValues => (oddValues % 2))
  return result;
...
const filterStringsWithVowels = (arr) => {
  let newArray = arr.filter(str => /[aeiou]/i.exec(str));
    return(newArray);
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  const newArray = arr.filter(notInFirstArray => {
  for(let i = 0; i < forbiddenValues.length; i++){
      if(notInFirstArray === forbiddenValues[i]){
      return false;
     } 
}
      return true;
