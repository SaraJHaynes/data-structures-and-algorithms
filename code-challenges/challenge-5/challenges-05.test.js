...
const splitFoods = (recipe) => {
  ...
  let splitUpArray = recipe.ingredients[i].split(' ');
  let slicedUpArray = splitUpArray.slice('2');
    let joinedString = slicedUpArray.join(' ')

  // Solution code here...
  result[i] = joinedString;
};
...
const stepActions = (recipe) => {
  ...
  for (let i = 0; i < recipe.steps.length; i++) {
      let splitUpArray = recipe.steps[i].split(' ');
    //   let slicedUpArray = splitUpArray.slice(0, 1);
      result[i] = splitUpArray[0];
  };
...
//     let result = [];
//     for (let i = 0; i < recipe.steps.length; i++) {
//       let splitArray = recipe.steps[i].split(' ');
//       let sliceArray = splitArray.slice(0,1);
