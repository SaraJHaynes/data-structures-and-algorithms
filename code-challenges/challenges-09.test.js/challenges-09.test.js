...
const countNumberOfElements = (arr) => {
  let length;
//   return arr.reduce((acc, val, idx) => length = idx + 1);
// };
...
const reversedString = (str) => {
  ...
  let reversed = strArray.reduce((acc, val, idx) => {

    return val + acc;
    
    // return str.split('').reduce((acc, val, idx) => val + acc, '')
...
const countNumberOfChildren = (arr) => {
    let lengthCount = [];
    arr.reduce((acc, val, idx) => {
        return lengthCount = val.children + idx;
    }, 0)
